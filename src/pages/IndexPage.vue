      ...
      <div class="container">
        <div class="calculator-inner">
          ...
          <ul class="box-list">
            ...
            <li
              class="box-list-item"
              :class="{ boxlistitemactive: isActiveras }"
            >
            ...
            <div class="calculator-box-wrapper">
              <div class="calculator-box-inner">
                <form class="form" method="post" autocomplete="off">
                  ...
                  <div
                    class="cvadrat-group"
                    :class="{ cvadratgroupwater: litr }"
                  >
                    <input
                      class="cvadret-input ponel-input"
                      v-model="kvMeter"
                      id="kvMeter"
                      type="number"
                      :placeholder="$t('power')"
                    />
                    <input
                      class="cvadret-input wotter-input"
                      v-model="watterMeter"
                      id="watterMeter"
                      type="number"
                      :placeholder="$t('volume_in_liters')"
                    />
                    <button class="cvadret-btn" @click="calcs()" type="button">
                      {{ $t("calculate") }}
                    </button>
                  </div>
                  ...
                  <p class="summa">
                    {{ $t("amount_compensation")
                    }}<strong id="resultSum">{{ testResult }}</strong>
                  </p>
              ...
              <div class="calculator-box-inner">
                <form class="form" method="post" autocomplete="off">
                  ...
                  <div class="cvadrat-group" id="cvadrat">
                    <input
                      class="cvadret-input intl-number-input"
                      v-model="raseum"
                      id="rasInput"
                      type="number"
                ...
                <table class="table">
                  ...
                  <tbody class="table__body">
                    <tr v-for="(menu, idx) in tabelres" :key="idx">
                      <td>{{ idx + 1 }}</td>
                      <td>{{ $t("months") }}</td>
                ...
                <p class="itogo">{{ $t("total") }}</p>
                <p class="summa">
                  <strong>{{ $t(elrasresultSum) }}</strong>
                </p>
              </div>
            </div>
          </div>
...
<script setup>
import { GetProductApi } from "src/API/API";
import { ref } from "vue";
...
const getProductApi = async () => {
  const cardCasts = await GetProductApi();
